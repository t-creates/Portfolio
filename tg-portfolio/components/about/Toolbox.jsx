          ...
          >
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {techList.map((tech) => {
                ...
                return (
                  <div key={tech._id || tech.name} className="flex items-center gap-4 rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm text-black/70 shadow-sm">
                    <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl border border-black/5 bg-gradient-to-br from-green-50 to-slate-100 text-2xl text-slate-800">
                      {tech?.image ? (
                        <img src={urlFor(tech.image)} alt={tech.name} className="h-8 w-8 object-contain" />
                      ) : (
                        <Icon aria-hidden="true" />
                      )}
                    </div>
                    <div className="flex flex-col">
                      <span className="font-semibold text-base text-black">{tech.name}</span>
